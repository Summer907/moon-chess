...
The frontend has three views: `MoonHallView` (route `/`, the mode-selection landing page), `SilverMoonTeaPartyView` (route `/tea-party`, the single-player-vs-AI play view), and `LunarOrbitView` (route `/lunar-orbit`, an analysis/simulation sandbox). The tea-party view currently has no multiplayer mode planned. Keep route titles and favicons in `frontend/src/router.ts`. Both game views share `ResponsiveDisclosure` for desktop/mobile panels and persist their independent start settings through `utils/useGamePreferences.ts`; preserve those shared paths instead of duplicating responsive or storage logic in a view.
